...
def build_schedule(
    ...
    rows: list[ScheduleRow] = []
    saldo = pv
    data_venc = data_liberacao + timedelta(days=d1)
...
@given(
    ...
    taxa=st.decimals(min_value=Decimal("0.001"), max_value=Decimal("0.05"), places=6),
...
from app.core.iof import IofConfig, compute_iof
...
```python
from app.core.iof import compute_financed_amount_with_iof
...
def _valor_por_parcela(extra: Extra) -> Decimal:
    ...
    if extra.modalidade is ExtraModalidade.UNICO_INICIAL:
        return quantize_brl(extra.valor_total)
    raise ValueError(f"Unknown modalidade: {extra.modalidade}")
...
def apply_extraordinary_amortizations(
    ...
    """
    rows = list(schedule_original.rows)
    ...
    total_pago = sum((r.parcela for r in rows), start=Decimal("0"))
    total_juros = sum((r.juros for r in rows), start=Decimal("0"))
    new_pmt_out = rows[-1].parcela if rows else pmt_original
    return Schedule(rows=rows, pmt=new_pmt_out, total_pago=total_pago, total_juros=total_juros)
...
def _rebuild_segment(
    ...
    # Renumber rows to continue numbering
...
- [ ] PR or branch merged to `main` with green CI
