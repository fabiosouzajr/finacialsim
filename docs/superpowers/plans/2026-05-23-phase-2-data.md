...
**Goal:** Implement the persistence layer — SQLAlchemy 2.x models for all 13 tables from spec §5, Alembic migrations, repositories, SQLite backup, and seed data.
...
```python
"""SQLAlchemy engine & session factory for FinacialSim (SQLite)."""
...
def create_engine_for_sqlite(db_path: Path) -> Engine:
    """Create a SQLite engine with WAL + FK pragmas enabled."""
...
def test_create_client_with_creator(session):
...
`app/data/models.py`:
```python
"""SQLAlchemy ORM models for FinacialSim."""
...
def test_create_simulation_with_extras_and_fees(session):
    ...
    fee = SimulationFee(simulation_id=sim.id, nome="Cadastro", valor=Decimal("500"), incluir_no_principal=True)
...
def test_amortization_row_persists(session):
    ...
    session.commit()
    row = AmortizationRow(
        simulation_id=sim.id, numero_parcela=1, data_vencimento=date(2026, 1, 31),
...
class AmortizationRow(Base):
    ...
    extras_total: Mapped[Decimal] = mapped_column(Numeric(18, 4), default=0, nullable=False)
    parcela_total: Mapped[Decimal] = mapped_column(Numeric(18, 4), default=0, nullable=False)
    ajuste_arredondamento: Mapped[Decimal] = mapped_column(Numeric(18, 4), default=0, nullable=False)
...
class ExtraordinaryAmortization(Base):
    ...
    tipo: Mapped[str] = mapped_column(String(20), nullable=False)
    aplicado_em: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
```
...
def test_business_rule_unique_chave(session):
...
def test_indicator_history_unique_codigo_data(session):
    i = IndicatorHistory(
...
def test_audit_log(session):
    log = AuditLog(timestamp=datetime.utcnow(), acao="login")
    session.add(log)
    session.commit()
    assert log.id is not None


def test_app_setting(session):
...
class Proposal(Base):
    ...
    gerado_em: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
...
from app.data.database import Base
from app.data import models  # noqa: F401 -- ensure models are imported
...
def _resolve_url() -> str:
    """Allow URL override via `-x db_url=...` (used by main.py at startup)."""
...
Expected: a new file in `app/data/migrations/versions/<DATE>_<rev>_initial_schema.py` containing `op.create_table(...)` for all 14 tables.
...
This creates a new file. Replace its `upgrade()` and `downgrade()` with:
...
down_revision = "<previous rev id from initial migration>"
branch_labels = None
depends_on = None
...
> Replace `<auto-generated>` and `<previous rev id...>` with the actual values Alembic generated.
...
def test_list_active_only(session):
    repo = UserRepository(session)
...
```python
"""Repositories - typed CRUD for ORM models."""
...
def test_client_create_and_find_by_cpf(session):
    from app.data.repositories import UserRepository
...
def test_indicator_upsert(session):
...
def test_business_rule_get(session):
...
from app.data.models import BusinessRule, Client, IndicatorHistory
...
class ClientRepository:
    ...
    def create(self, nome: str, cpf_cnpj: str, tipo: str, criado_por: int, **kwargs) -> Client:
        c = Client(nome=nome, cpf_cnpj=cpf_cnpj, tipo=tipo, criado_por=criado_por, **kwargs)
        self.session.add(c)
        self.session.commit()
...
class IndicatorRepository:
    ...
    def upsert(self, codigo: str, data_referencia: _date, valor: _Decimal,
               unidade: str, fonte: str, payload_json: str | None = None) -> IndicatorHistory:
...
class BusinessRuleRepository:
    ...
    def set(self, chave: str, valor_json: str, descricao: str | None = None,
            user_id: int | None = None) -> BusinessRule:
        stmt = select(BusinessRule).where(BusinessRule.chave == chave)
        existing = self.session.scalars(stmt).first()
        if existing:
...
git commit -m "feat(data): Client, Indicator, BusinessRule repositories"
```
...
def test_backup_creates_gz_file():
    with tempfile.TemporaryDirectory() as tmp:
        db_path = Path(tmp) / "test.db"
        engine = create_engine_for_sqlite(db_path)
        Base.metadata.create_all(engine)
        ...
        # gzipped sqlite signature: gunzip header is fine
        with gzip.open(backup_file, "rb") as f:
            header = f.read(16)
        assert header.startswith(b"SQLite format 3")


def test_list_backups_returns_chronological():
    with tempfile.TemporaryDirectory() as tmp:
        db_path = Path(tmp) / "test.db"
        engine = create_engine_for_sqlite(db_path)
        Base.metadata.create_all(engine)
...
def test_restore_overwrites_target():
    with tempfile.TemporaryDirectory() as tmp:
        db_path = Path(tmp) / "test.db"
        engine = create_engine_for_sqlite(db_path)
        Base.metadata.create_all(engine)
        ...
        # Size should be approximately equal (gz roundtrip)
...
`app/data/backup.py`:
```python
"""SQLite backup using sqlite3.Connection.backup() + gzip."""
...
def test_alembic_upgrade_and_seed():
    """Run alembic upgrade head against a temp DB and verify seed data."""
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        # Copy alembic.ini + migrations to tmp; rewrite sqlalchemy.url
        shutil.copy("alembic.ini", tmp_path / "alembic.ini")
        shutil.copytree("app", tmp_path / "app")

        db_file = tmp_path / "test.db"
        ini_text = (tmp_path / "alembic.ini").read_text()
        ini_text = ini_text.replace(
            "sqlalchemy.url = sqlite:///data/finacialsim.db",
            f"sqlalchemy.url = sqlite:///{db_file}",
        )
        (tmp_path / "alembic.ini").write_text(ini_text)
        ...
        # Verify admin user + at least 10 business rules exist
        import sqlite3
...
- [ ] All 14 entity tables + `alembic_version` present after upgrade
- [ ] Seed admin user + 18+ business_rules exist after seed migration
- [ ] All `pytest` tests pass (`tests/unit/data/` and `tests/integration/`)
- [ ] `mypy app/data/` passes (non-strict on this layer)
